    ...
    def get(self, request, category_name_slug):
        ...
        if query == None:
            try:
                ...
                pages = Page.objects.all().filter(category=category).order_by('-likes')               
                videos = Video.objects.filter(category=category).order_by('-likes')
            ...
            category = Category.objects.get(slug=category_name_slug)
            pages = Page.objects.all().filter(category=category).filter(title__contains=query).order_by('-likes')
            videos = Video.objects.all().filter(category=category).filter(title__contains=query).order_by('-likes')
